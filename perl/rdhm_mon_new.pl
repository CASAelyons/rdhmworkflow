...
sub file_monitor {
	...
	if (($qpefiles_contained == 0) && (@qpefiles)) {
	    ...
	    my $wfcall = "python3.6 /home/ldm/rdhmworkflow/run_rdhm_new.py -s " . $starttime . " -f " . $endtime . " -i Realtime_RSRT2_CASA_container.card";
	    print $wfcall . "\n";
	    system($wfcall);
	...
	foreach $file (@new_data_files) {
		...
		my $unzipped_del_file = "/nfs/shared/rdhm/output/" . $typesplit[0] . "/". $short_filename;
	...
	#	}
		#unlink($file);
	#    }
	    elsif ($pathsuffix eq "netcdf") {
		my $gzip_call = "/bin/gzip " . $file;
		system($gzip_call);
	    }
	}
    }
